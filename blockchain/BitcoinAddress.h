...
bool bitcoinPublicKeyToAscii(uint8_t pubkeyAddress, const uint8_t input[65], // The 65 bytes long ECDSA public key; first byte will always be 0x4 followed by two 32 byte components
...
bool bitcoinPublicKeyToHash160(const uint8_t input[65], // The 65 bytes long ECDSA public key; first byte will always be 0x4 followed by two 32 byte components
							   uint8_t output[20]);		// A bitcoin Hash160
...
bool bitcoinAddressToHash160(const uint8_t input[25], uint8_t output[20]);
