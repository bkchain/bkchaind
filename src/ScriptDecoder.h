...
class ScriptDecoder
{
	const uint8_t* scriptStart;
	const uint8_t* script;
	const uint8_t* endOfScript;

public:
	ScriptDecoder(const uint8_t *script, uint32_t scriptLength)
		: scriptStart(script), script(script), endOfScript(script + scriptLength)
	{
	...
	bool readOpcode(uint8_t& opcode, bool skipData = false)
	{
		if (script >= endOfScript)
			return false;

		opcode = *script++;
		if (opcode < OP_PUSHDATA1 && skipData)
		{
			script += opcode;
		}
		return true;
	...
	bool readData(uint8_t length, uint8_t* data)
	{
		if (script + length > endOfScript)
			return false;
		memcpy(data, script, length);
		script += length;
		return true;
	}
};
